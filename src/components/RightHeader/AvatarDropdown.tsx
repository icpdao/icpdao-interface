...
export const LoginButton: React.FC = () => {
  ...
  return (
    <div className={styles.loginButton}>
      <Button href={githubOAuth} icon={<GithubOutlined style={{fontSize: 17}} />} block>{loginText}</Button>
    </div>
  );
}
...
const AvatarDropdown: React.FC<GlobalHeaderRightProps> = () => {
  ...
  return (
    <HeaderDropdown overlay={menuHeaderDropdown} trigger={['hover', 'click']}>
      <span className={`${styles.action} ${styles.account}`}>
        <Avatar size={40} className={styles.avatar} src={initialState.currentUser().profile.avatar} alt="avatar" />
        <span className={`${styles.name} anticon`}>{initialState.currentUser().profile.nickname}</span>
        <DownOutlined />
      </span>
    </HeaderDropdown>
  );
