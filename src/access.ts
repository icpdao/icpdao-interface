...
export default function access(initialState: {currentUser: any}) {
  ...
  return {
    ...
    canInviteIcpper: () => (currentUser().profile?.icppership?.progress !== 0 &&
      currentUser().profile?.icppership?.progress !== undefined) || currentUser().profile?.status === 2,
    hadMentor: () => currentUser().profile?.icppership?.progress !== 0 &&
