...
export default (): ReactNode => {
  const intl = useIntl();
  const {event$, isConnected, contract, accounts, network, metamaskProvider} = useModel('useWalletModel')
  ...
  const handlerDeposit = useCallback(async () => {
    try {
      ...
      const tx = await contract.daoStaking.deposit(depositInputValue.toString(), tokenList.map((t) => t.address))
  ...
  const handlerBonusPreview = useCallback(async () => {
    ...
    for (let i = 0; i < tokens.length; i+=1) {
      ...
      ta.push({amount: amounts[i], tokenAddress: tokens[i], tokenSymbol: await (new ERC20Connect(tokens[i], network, metamaskProvider)).getTokenSymbol()})
    }
  ...
  const handlerTokenSearch = useCallback(async (value) => {
    if (!value) {
      return
    }
        ...
        <Row>
          <Col span={14} offset={2}>
            <Row className={styles.StakingTitle}>
              <div><FormattedMessage id={`pages.staking.title`} /></div>
            </Row>
            <Row className={styles.StakingDesc}>
              <div><FormattedMessage id={`pages.staking.desc`} /></div>
            </Row>
                  ...
                  >
                    <Form layout={'vertical'} name={'stakeForm'}>
                      ...
                      <Form.Item>
                        ...
                        {isConnected ? (icpdApproved ? <Button onClick={handlerDeposit} loading={loadingStakeButton} size={'large'} style={{width: '100%'}} type={'primary'} disabled={depositInputValue.eq(BigNumber.from(0))}>
                          <FormattedMessage id={`pages.staking.button.stake`} />
                        </Button> : <Button onClick={handlerApprovedICPD} loading={loadingApproveButton} size={'large'} style={{width: '100%'}} type={'primary'}>
                          <FormattedMessage id={`pages.staking.button.approve`} />
                        </Button>) : <Button type="primary" size={'large'} style={{width: '100%'}} onClick={() => handlerMetamaskConnect()}>
                ...
                <TabPane tab={<FormattedMessage id={`pages.staking.tab.un_stake`} />} key="unStake" >
                  ...
                  >
                    <Form layout={'vertical'} name={'unStakeForm'}>
                      ...
                      <Form.Item>
                        {isConnected ? <Button onClick={handlerWithdraw} loading={loadingUnStakeButton} size={'large'} style={{width: '100%'}} type={'primary'} disabled={unDepositInputValue.eq(BigNumber.from(0))}>
          ...
          <Col span={6}>
            ...
            <Row className={styles.StakingWithdrawButton}>
              <Button onClick={handlerBonusPreview} loading={loadingWithdrawButton} size={'large'} type={'primary'} style={{width: '100%'}} >
            ...
            <Row>
              <Form layout={'vertical'} style={{width: '100%'}}  name={'tokenListForm'}>
                ...
                >
                  <Form.Item>
                    <List
                      header={<div>{intl.formatMessage({id: 'pages.staking.select.token_list'})}</div>}
                      bordered
                      size={'small'}
                      ...
                      renderItem={item => (
                        <List.Item actions={[<Button disabled={cacheAddTokenList.length > 0} type={'link'} onClick={() => {
                          setCacheRemoveTokenList(cacheRemoveTokenList.concat([item.address]))
                  ...
                  <Form.Item>
                    ...
                    >
                      {cacheTokenList.map((t) => <Select.Option key={t.address} value={t.address} tokensymbol={t.symbol}>
                        <Space>
                  ...
                  <Form.Item>
                    {cacheAddTokenList.length > 0 && <Button type={'primary'} onClick={handlerAddTokenList}>Add Token</Button>}
                    {cacheRemoveTokenList.length > 0 && <Button type={'primary'} onClick={handlerRemoveTokenList}>Remove Token</Button>}
                  </Form.Item>
                </Spin>
              </Form>
            </Row>
