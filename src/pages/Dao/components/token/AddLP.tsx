...
const TokenAddLP: React.FC<TokenConfigComponentsProps> = ({tokenAddress}) => {
  ...
  const handlerAddLP = useCallback(async () => {
    ...
    const quoteTokenAmount = poolInfo.invertPrice ? position.mintAmounts.amount0.toString() : position.mintAmounts.amount1.toString();
  ...
  return <>
    ...
    >
      <Form layout={"vertical"} name={'tokenAddLP'}>
        <Form.Item
          label={intl.formatMessage({ id: 'pages.dao.config.tab.token.add_lp.form.total_amount' })}
        ...
        <Form.Item
          label={intl.formatMessage({ id: 'pages.dao.config.tab.token.add_lp.form.base_token_amount' })}
          tooltip={intl.formatMessage({
        ...
        >
          <InputNumber
            ...
            onChange={(value) => setFormDataFast({
        ...
        <Form.Item>
          <Switch
            ...
            onChange={(value) => {
              ...
              setFormDataFast({minPrice: formData.startingPrice?.toString() || true, maxPrice: true})
            }}
            disabled={!formData.startingPrice}
            checkedChildren={intl.formatMessage({id: 'pages.dao.config.tab.token.add_lp.form.advanced'})}
            unCheckedChildren="" />
        </Form.Item>
        {advancedOP && <Row>
          <Space>
            <Form.Item
              label={intl.formatMessage({ id: 'pages.dao.config.tab.token.add_lp.form.min_price' })}
              tooltip={intl.formatMessage({
            ...
            >
              <InputNumber
                ...
                onChange={(value) => {
                  setFormDataFast({minPrice: value?.toString() || formData.startingPrice?.toString() || true})
                }}
                onStep={(_, info) => {
            ...
            <Form.Item
              label={intl.formatMessage({ id: 'pages.dao.config.tab.token.add_lp.form.max_price' })}
              tooltip={intl.formatMessage({
        ...
        <Form.Item>
          {(metamaskIsConnected && !!formData.quoteToken && !!formData.fee) && <Button
            type="primary"
