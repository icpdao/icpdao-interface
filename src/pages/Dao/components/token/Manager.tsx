...
const TokenManager: React.FC<TokenConfigComponentsProps> = ({tokenAddress}) => {
  ...
  const handlerRemoveManager = useCallback(async (managerAddress: string) => {
    if (!tokenContract) return
    try {
  ...
  return <>
    ...
    >
      <List
        header={<div>{intl.formatMessage({id: 'pages.dao.config.tab.token.manager.list.header'})}</div>}
        bordered
        style={{width: 800, marginBottom: 20}}
        itemLayout="horizontal"
        dataSource={managers}
        renderItem={item => (
          <List.Item actions={[<Button type={'link'} loading={removeButtonLoading[item]} onClick={() => handlerRemoveManager(item)} ><DeleteOutlined /></Button>]}>
            {item}
          </List.Item>
        )}
      />
      ...
      >
        ...
        >
          <Input
            style={{width: '400px'}}
            value={addManager}
            onChange={(value) => setAddManager(value.target.value)}
          />
        </Form.Item>
        <Form.Item>
          {isConnected ? <Button
            type={'primary'}
            onClick={handlerAddManager}
            loading={addButtonLoading}>
            {intl.formatMessage({id: 'pages.dao.config.tab.token.manager.form.button.add'})}</Button> : <Button
            type="primary"
