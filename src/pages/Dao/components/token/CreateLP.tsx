...
import {Form, Spin, Input, Space, Alert, Select, Radio, InputNumber, Switch, Row, Button, Descriptions} from "antd";
...
const getCoinGecko = async (network: string) => {
  ...
  compoundTokenList.tokens.forEach((t) => {
    if (t.chainId === chainId) {
      quoteTokens[t.address] = new Token(
        t.chainId, t.address, t.decimals, t.symbol, t.name
      )
    }
  })
  return quoteTokens
...
const TokenCreateLP: React.FC<TokenConfigComponentsProps> = ({tokenAddress}) => {
  ...
  const [approveQuoteTokenButtonLoading, setApproveQuoteTokenButtonLoading] = useState<boolean>(false);
  ...
  useMemo(async () => {
    ...
    const up = await contract.uniswapPool.getPool(formData.baseToken, formData.quoteToken, formData.fee)
    setPoolInfo(up);
  ...
  const maxBaseTokenAmount = useMemo(async () => {
    ...
    return parseInt(formatUnits(amount), 10)
  }, [tokenContract])
  ...
  const maxQuoteTokenAmount = useMemo(async () => {
    ...
    const quoteTokenContract = new ERC20Connect(formData.quoteToken.address, network, metamaskProvider)
    ...
    return parseInt(formatUnits(amount, decimals), 10)
  ...
  useMemo(async () => {
    ...
    const {decimals} = quoteTokenSelectObject[formData.quoteToken.address]
    console.log({allowance: parseInt(formatUnits(allowance, decimals), 10)})
    setApprovedQuoteToken(parseInt(formatUnits(allowance, decimals), 10) >= formattedAmounts[UniswapField.CURRENCY_B]);
  }, [accounts, formData.quoteToken, formattedAmounts, metamaskProvider, network, quoteTokenSelectObject, tokenAddress])
  ...
  const handlerCreateLPPool = useCallback(async () => {
    ...
    const createLPPool: ETH_CONNECT.CreateLPPool = {
      baseTokenAmount: poolInfo.invertPrice ? position.mintAmounts.amount1.toString() : position.mintAmounts.amount0.toString(),
      quoteTokenAddress: poolInfo.invertPrice ? position.pool.token0.address : position.pool.token1.address,
      quoteTokenAmount: poolInfo.invertPrice ? position.mintAmounts.amount0.toString() : position.mintAmounts.amount1.toString(),
    ...
    // TODO: get from event
    setLPPoolAddress(await tokenContract.getLPPool());
  }, [accounts, network, position, tokenContract])
    ...
    >
      <Form layout={"vertical"} name={'tokenCreateLP'}>
        <Space>
          ...
          >
            <Input style={{width: '200px'}} disabled={true} value={formData.baseToken?.name || ''} />
          </Form.Item>
          <Form.Item
          ...
          >
            <Select
              ...
              onChange={async (value) => {
                ...
                setFormDataFast({fee: undefined, startingPrice: 0, baseTokenAmount: 0, quoteTokenAmount: 0, quoteToken});
              }}
              style={{width: '200px'}}
            >
              {quoteTokenSelectList.map((t) => <Select.Option key={t.address} value={t.address} tokensymbol={t.symbol}>
                <Space>
        ...
        <Form.Item
          label={intl.formatMessage({ id: 'pages.dao.config.tab.token.create_pool.form.set_starting_price' })}
          tooltip={intl.formatMessage({
        ...
        <Form.Item>
          <Switch
            ...
            checkedChildren={intl.formatMessage({id: 'pages.dao.config.tab.token.create_pool.form.advanced'})}
            unCheckedChildren="" />
        </Form.Item>
        {advancedOP && <Row>
          <Space>
            <Form.Item
              label={intl.formatMessage({ id: 'pages.dao.config.tab.token.create_pool.form.min_price' })}
              tooltip={intl.formatMessage({
            ...
            <Form.Item
              label={intl.formatMessage({ id: 'pages.dao.config.tab.token.create_pool.form.max_price' })}
              tooltip={intl.formatMessage({
        ...
        <Row>
          <Space>
            <Form.Item
              label={intl.formatMessage({ id: 'pages.dao.config.tab.token.create_pool.form.base_token_amount' })}
              tooltip={intl.formatMessage({
            ...
            <Form.Item
              label={intl.formatMessage({ id: 'pages.dao.config.tab.token.create_pool.form.quote_token_amount' })}
              tooltip={intl.formatMessage({
        ...
        <Form.Item>
          {(metamaskIsConnected && !!formData.quoteToken && !!formData.fee) && (approvedQuoteToken ? <Button
            type="primary"
            disabled={!(formattedAmounts[UniswapField.CURRENCY_A] && formattedAmounts[UniswapField.CURRENCY_B])}
            onClick={() => setPreviewCreateLP(true)}>
            {intl.formatMessage({ id: 'pages.dao.config.tab.token.create_pool.form.button.create' })}
          </Button>: <Button
            type="primary"
            loading={approveQuoteTokenButtonLoading}
            disabled={!(formattedAmounts[UniswapField.CURRENCY_A] && formattedAmounts[UniswapField.CURRENCY_B])}
            onClick={() => handlerApprovedQuoteToken()}>
            {intl.formatMessage({ id: 'pages.dao.config.tab.token.create_pool.form.button.approve' }, {quote_token_name: formData.quoteToken?.symbol})}
          </Button>)}
          {!metamaskIsConnected && <Button
            type="primary"
            disabled={!(formattedAmounts[UniswapField.CURRENCY_A] && formattedAmounts[UniswapField.CURRENCY_B])}
            onClick={() => handlerMetamaskConnect()}>
            {intl.formatMessage({ id: 'pages.dao.config.tab.token.create_pool.form.button.connect' })}
          </Button>}
        </Form.Item>
      </Form>
    </Spin>
    ...
    >
      <Descriptions title="Preview Create LP Pool" bordered column={2}>
        <Descriptions.Item
          ...
          label={intl.formatMessage({ id: 'pages.dao.config.tab.token.create_pool.form.base_token' })}
        >
          {poolInfo.invertPrice ? position?.amount1.toSignificant(4) : position?.amount0.toSignificant(4)}
        </Descriptions.Item>
        <Descriptions.Item
          span={2}
          label={intl.formatMessage({ id: 'pages.dao.config.tab.token.create_pool.form.quote_token' })}
        >
          {poolInfo.invertPrice ? position?.amount0.toSignificant(4) : position?.amount1.toSignificant(4)}
        </Descriptions.Item>
        <Descriptions.Item
          span={2}
          label={intl.formatMessage({ id: 'pages.dao.config.tab.token.create_pool.form.fee' })}
        ...
        <Descriptions.Item
          span={2}
          label={intl.formatMessage({ id: 'pages.dao.config.tab.token.create_pool.form.current_price' })}
        >
